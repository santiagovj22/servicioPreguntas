...
router.get('/api/open_questions',checkAuth, async (req,res) => {
    try{
...
router.post('/api/delete_question/:id', checkAuth, async (req,res) => {
...
router.post('/api/create_answer/:id',checkAuth, async (req,res) => {
   questionid = req.params.id;
    try {
...
router.get('/api/report',checkAuth, async (req,res,next) => {
    try{
       let respuesta= await questions.report();
        res.json({data:respuesta}).status(200);
    } catch(err){
        res.json({error:true,success:false, message: 'Error'}).status(400);
    }
})
